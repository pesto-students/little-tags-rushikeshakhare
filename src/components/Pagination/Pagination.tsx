...
export const Pagination = ({ totalPages, onPageSelected, navigationSize = 5 }: IPagination) => {
    ...
    return (
        <div className="pagination item-center">
            <div className="pagination-nav previous left">
                <button onClick={() => handleNavigation(-1)} className="btn-flat">{'<<'} Previous</button>
            </div>
            <div className="pagination-nav next right">
                <button onClick={() => handleNavigation(1)} className="btn-flat">Next {'>>'}</button>
            </div>
...

                {
                    range(navigationSize).map((page, index) => (
                        <li key={index} className="pagination-list-item">
                            <button onClick={() => handlePageSelect(page + offset + 1)} className={`btn-flat ${selectedPage === page + offset + 1 ? 'selected': ''}`}>{page + offset + 1}</button>
                        </li>
                    ))
                }
