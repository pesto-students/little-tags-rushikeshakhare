...
export const WishListItem = ({ id, name, image, price, deleteEvent }: IWishListItem) => {
  ...
  return (
    <div className="wish-list-item item-center d-flex">
      ...
      <button onClick={(e) => handleDelete(e)} className="btn-flat wish-list-item-delete-btn"><img alt="wishlist delte item button" src={iconDelete}></img></button>
      <div className="wish-list-item-text left">{name}</div>
      <div className="wish-list-item-price right text-right"> ₹ {price} </div>
    </div>
  );
};
